...
class SimpleSiena: public virtual Siena {
 public:
	...
	notifications.
    */
			SimpleSiena(const string &siena_uri,
				    INAddress::port_type p = 0);
			~SimpleSiena();

    virtual void	publish(const Pub &);

    virtual void	subscribe(const Sub &, Notifiable *);
    virtual void	unsubscribe(const Sub &, Notifiable *);
 ...
 private:
    URI				server;
    INAddress::port_type	port_number;
    bool			in_loop;
    TCPPort *			port;
    string *			myuri;

    void			sub_uns(const string & method, const Sub & f, Notifiable *party);
};
